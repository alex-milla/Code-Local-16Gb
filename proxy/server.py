#!/usr/bin/env python3
"""
MLX Native Anthropic Server — Claude Code on Apple Silicon (16GB Edition)
...
Pick a model from the 16GB-friendly lineup with the MLX_MODEL env var:
    MLX_MODEL=mlx-community/phi-4-4bit                          (RECOMMENDED — low hallucinations, fast)
    MLX_MODEL=mlx-community/Qwen3-14B-4bit                      (Great context handling)
    MLX_MODEL=mlx-community/Qwen2.5-Coder-14B-Instruct-4bit     (Code-specialized)

For 16 GB Macs, you may want to enable KV-cache quantization to save memory
on long conversations:
    MLX_KV_BITS=8 MLX_MODEL=mlx-community/phi-4-4bit python3 proxy/server.py
...
def optimize_for_code(body):
    ...
    slim Llama-tuned coding prompt and filter tools down to the core 6."""
